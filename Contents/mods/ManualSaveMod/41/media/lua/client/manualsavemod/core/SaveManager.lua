...
function ManualSave.SaveManager.load(gmode, world, slot, onDone)
    ...
    -- Performance note (1.5.3): the unconditional `pcall(save, true)` that used
    -- to live here added several seconds of latency to every in-game Load
    -- action — including the case where the user just wants to switch to a
    -- different save. The flush is only required for the "same-world while
    -- in-game" branch below (so PZ does not overwrite the backup we just
    -- restored with stale in-memory state). In every other case we skip it
    -- and the Load opens immediately.
        ...
        function(status, result)
            if status == "OK" then
                ...
                if inGame and ok and liveWorld == slot then
                    -- Flush in-memory state ONLY here, then quit so PZ
                    -- restarts clean from disk where the backup is.
                    pcall(save, true)
                    LT.clearSessionFile()
