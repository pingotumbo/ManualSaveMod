...
-- Checkbox toggle row: checkbox + title + (wrapped) description + bottom
-- separator. The description wraps to the row width and the row auto-sizes its
-- height to fit, so long translations never get clipped.
...
function ManualSave.makeSettingsCheckRow(parent, opts)
    ...
    -- Wrap the description into the text column (leave a right margin for the
    -- scrollbar). descW can be <=0 on the first frame if width is unknown; wrapText
    -- then returns the text as-is and we recompute next layout pass.
    local descW     = (opts.w or 0) - textX - 16
    local descLines = ManualSave.wrapText(desc, UIFont.Small, descW)
    local ROW_H     = math.max(opts.h or 0, 4 + FHS + 3 + #descLines * lineH + 7)
